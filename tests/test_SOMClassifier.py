...
def test_init_super_som(n_rows, n_columns, random_state):
...
@pytest.mark.parametrize("n_rows,n_columns,learningrate,dist_weight_matrix,random_state,class_weight,expected", [
...
def test_change_class_proba(n_rows, n_columns, learningrate,
                            dist_weight_matrix, random_state, class_weight, expected):
    som = susi.SOMClassifier(n_rows=n_rows, n_columns=n_columns,
                                     random_state=random_state)
    # som.classes_ = [0, 1, 2]
    # som.class_weights_ = [1., 1., 1.]
    new_som_array = som.change_class_proba(learningrate, dist_weight_matrix, class_weight)
